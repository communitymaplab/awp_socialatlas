...
#data <- read.csv("data/GA_2020/GA_acs")
...
#write.csv(df_data, paste("data/GA_", yr, "/GA_acs.csv", sep = "")) #optional download of full state data
  
  acc_acs <- df_data %>%
    filter(substr(GEOID,1,5)=="13059") 

write.csv(acc_acs, paste("data/GA_", yr, "/ACC_acs.csv", sep = ""))
...
st_write(state_geom %>%
  ...
  census_data_all_long<-rbind(census_data_est_only,census_data_pct_only) %>%
    rename("var"=var_group) %>%
    gather(est:moe,key="var_type",value="value") %>%
    filter(substr(GEOID,1,5)=="13059")

write.csv(census_data_all_long, paste("data/GA_", yr,"/long_acc_tractdata.csv", sep = ""))
