...
#               variable = acs_vars, year = 2018)
# write_csv(df,"data/ACS_data.csv")

df<-read_csv("data/ACS_data.csv") %>%
...
df_geom<-st_read("data/ACStracts_2016.geojson") %>%
  mutate(GEOID=as.character(GEOID)) %>%
  select(GEOID)
```
...
write_csv(census_data_all,"data/census_tractdata_all.csv")

#Clarke data
census_data_acc<-census_data_all %>%
  filter(substr(GEOID,1,5)=="13059")
write_csv(census_data_all,"data/census_tractdata_acc.csv")
```
