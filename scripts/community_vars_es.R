...
avg_ppsqft_change <- avg_ppsqft_2000 %>%
  left_join(avg_ppsqft_2010, by = "School") %>%
  left_join(avg_ppsqft_2020, by = "School") %>%
...
med_ppsqft_change <- med_ppsqft_2000 %>%
  left_join(med_ppsqft_2010, by = "School") %>%
  left_join(med_ppsqft_2020, by = "School") %>%
...
apprasial_data <- apprval_raw %>%
  ...
  select(PARCEL_NO, OWNER_NAME, `2022 Total`, `2022 Land`, `2022 Imp`, School, SchoolID, School, AREA) %>%
...
appr_totals <- apprasial_data %>%
  group_by(School) %>%
  mutate(School_apprtot = sum(appr_total)) %>%
  mutate(School_land = sum(land_area)) %>%
  select(School, School_land, School_apprtot) %>%
  unique() %>%
  mutate(tot_ppsqft = round((School_apprtot/School_land), digits = 2))
...
appraisals_wide <- appr_mean %>%
  left_join(appr_median, by = "School") %>%
  left_join(avg_appr_ppsqft, by = "School") %>%
  left_join(med_appr_ppsqft, by = "School")
...
business_distances_wide_es <- read_csv("data/accnaics_distances_es_wide.csv")

bus_dist_data <- business_distances_wide_es %>%
  select("School", "CONVENIENCE STORES", "SUPERMARKETS AND OTHER GROCERY (EXCEPT CONVENIENCE)",
...
write_csv(community_data, "data/community_data_es.csv")
